...
const RootComponent = (props) => {
  ...
  const addProductToCart = (product) => {
    ...
    newCart.products.find((obj) => obj.id === product.id).qty += 1;

    let newQty = newCart.products.find((obj) => obj.id === product.id).qty;

    newCart.products.find((obj) => obj.id === product.id).price =
      newQty * product.price;

    function getPrices(input, field) {
      let output = [];
      for (let i = 0; i < input.length; ++i) output.push(input[i][field]);
      return output;
    }

    const arrPrices = getPrices(newCart.products, "price");
    newCart.totalPrice = arrPrices.reduce((a, b) => a + b, 0);
  ...
  const removeProductFromCart = (product) => {
    ...
    newCart.products.find((obj) => obj.id === product.id).qty > 0
      ? (newCart.products.find((obj) => obj.id === product.id).qty -= 1)
      : (newCart.products.find((obj) => obj.id === product.id).qty = 0);
