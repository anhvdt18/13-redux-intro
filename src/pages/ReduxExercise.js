...
const CartPage = (props) => {
  // Step 6
  // Replace the line below to get data of the second product (Tuan Anh: two products ????) from state.cart.totalPrice
  const price = (state) => state.cart.totalPrice;
  const newTotalPrice = useSelector(price);
...
const ProductOne = (props) => {
  ...
  const selectProduct = (state) => state.product[0];
...
const ProductTwo = (props) => {
  ...
  // You should see the price is updated
  const selectProduct = (state) => state.product[1];
  const selectedProduct = useSelector(selectProduct);
...
const CartProductOne = (props) => {
  ...
  const selectProducts = (state) => state.cart.products[0];
...
const CartProductTwo = (props) => {
  ...
  // Change the price of products in `service/cart/slice.js` to see the effect
  const selectProducts = (state) => state.cart.products[1];
  const productList = useSelector(selectProducts);
  const product = {
    price: productList.price,
    qty: productList.qty,
